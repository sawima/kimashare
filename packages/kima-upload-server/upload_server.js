...
FileInfo.prototype.initUrls = function (req, form) {
  if (!this.error) {
    ...
    this.url = baseUrl + (subDirectory ? (subDirectory + '/') : '') + encodeURIComponent(this.key);

    // image
    Object.keys(options.imageVersions).forEach(function (version) {
      if (_existsSync(
          options.uploadDir + '/' + version + '/' + that.key
        )) {
        that[version + 'Url'] = baseUrl + version + '/' +
        encodeURIComponent(that.key);
      }
    });
  }
};
...
UploadHandler.prototype.post = function () {
  ...
  form.on('fileBegin', function (name, file) {
    ...
    fileInfo.key = newFileName;
    fileInfo.path = folder + "/" + newFileName;
    fileInfo.folderpath=folder;
